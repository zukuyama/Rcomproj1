...
void set_newsettings(int fd) // mete settings novos na port
{
...
void set_newsettings_custom(int fd, float vtime, int vmin) // mete settings novos na port
{
    if ( tcgetattr(fd,&oldtermios) == -1) // save current port settings
    {
        perror("tcgetattr");
...
int send(int fd)
{
    ...
    set_newsettings_custom(fd, 0.1, 0); // atribui o descritor do ficheiro de serial port
...
int receive(int fd)
{
    ...
    set_newsettings_custom(fd, 0, 5); // atribui o descritor do ficheiro de serial port
